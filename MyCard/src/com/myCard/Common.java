...
public class Common {
	...
	public static List<Card> getBestAI(List<Card> curList,List<Card> oppo){
		...
		if(oppo==null)
		{
			...
			//主动出牌
			// myModel 是 model对象  showCardslList是要走的排   curList是自己已有的牌
	...
	public static void showCards(Model model,List<Card> to,List<Card> from){
		...
		// 有单出单 (除开3带，飞机能带的单牌)
		//.isSingleOpper()如果敌人只有一张牌，尽量不出单，出单也从最大的出返回Boolean
		//.getValueByName通过name返回值
	...
	///	System.out.println("运行bulidts方法");
		//装上轮出的牌name
//		List<String> list2 = new Vector<String>();
//		for (Card card : view.outList[0]){
//			list2.add(card.name);
//		}
//		
//		if(list2.size()==0){
//			for (Card card : view.outList[2]){
//				list2.add(card.name);
//			}
//			
//		}
...
//		System.out.println("装上轮出的牌大小为："+lis.size());
		//通过name得到card
//		List<Card> a1 = getCardsByName(view.playerList[1],"a1_9");
//		//获得我的牌型
//		CardType bType = Common.jugdeType(view.outList[1]);
//		System.out.println("我的的牌型为:"+cType);
		
//		for(int i=0,len=lis.size();i<len;i++){
//			System.out.println("for(int i=0,len=lis.size();i<len;i++)");
//			Card card=lis.get(i);
//			card.setLocation(card.width/2,card.height*card.height/2);
//		}
		...
		//curList是自己的牌 oppo是比较的牌
		if(lis != null){
				player = Common.getBestAI(view.playerList[1], lis);
				if(player!=null){
					ts1 =true;
						for(Card play:player){
							play.y -= play.height/3;
							play.clicked=true;
						}
				}
	//			view.outList[1].addAll(player);
		} if (lis.size()==0) {
			ts1=true;
			if(card != null){
		...
		if(!ts1)
		view.message[1]="请自行选牌！";
//	view.Sleep(1000);
		
		
	}
//		//获得上一轮出的牌型
//		CardType cType = Common.jugdeType(lis);
//		System.out.println("上一轮出的牌型为:"+cType);
		view.update();//重绘
	}

	// 取出提示最小的牌
		public Card getCardts() {
			Card card = null;
			int a = view.playerList[1].size() ; 
			//取到最小的牌
